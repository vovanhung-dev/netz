...
const onPickImage = (e: Event) => { const f = (e.target as HTMLInputElement).files?.[0]; if (!f) return; if (!f.type.startsWith('image/')) return Toast.fire({ icon: 'error', title: 'Not an image' }); imageFile.value = f, preview.value = URL.createObjectURL(f) }
...
const submit = async () => {
  ...
  try {
    ...
    if (imageFile.value) {
      ...
      res = await http.post('/api/admin/courses', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    } else {
      res = await http.post('/api/admin/courses', payload)
    }
