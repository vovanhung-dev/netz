<!-- <template>
  <div class="detail-course">
    ...
    <img v-if="course?.cover_image_src" :src="course.cover_image_src" class="cover" />
    <div class="prose" v-html="course?.body"></div>
  </div>
</template>
...
async function load() {
  const res = await http.get(`/api/admin/courses/${props.id}`)
...
const load = async () => {
  const { data } = await http.get(`/api/admin/courses/${ id.value }`)
...
const submit = async () => {
  ...
  try {
    ...
    if (imageFile.value) {
      ...
      res = await http.post(`/api/admin/courses/${ id.value }`, fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    } else {
      res = await http.put(`/api/admin/courses/${ id.value }`, payload)
    }
